...
Snapshot at the tip of `audit-deepening-2026-08-20`. Regenerated by
`scripts/validate_metadata.py` (per-category finding counts) and by
the coverage-matrix generator at
[`reports/2026-08-20__audit-deepening__coverage-matrix.md`](reports/2026-08-20__audit-deepening__coverage-matrix.md).

- **Findings on-disk (7 total):**
  - `findings/correctness/` (1)
  - `findings/compile-time/` (2)
  - `findings/runtime/` (0)
  - `findings/duplication/` (0)
  - `findings/upstream-fragility/` (4)
  - `findings/test-gaps/` (0)
  - `findings/maintainability/` (0)
- **Scanners producing candidates:** 8 (`graph_mutations`,
  `private_api`, `repeated_analysis`, `list_surgery`, `workarounds`,
  `test_smells`, `runtime_artifact`, `pass_dependency_graph`).
- **Contracts populated:** 4 (`dependency-extraction`,
  `graphlowering`, `computed-buffer`, `pass-matrix`).
- **Pod-side follow-ups queued:** 5 (see `needs-pod/`).
